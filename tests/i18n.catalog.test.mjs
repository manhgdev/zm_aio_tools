...
test('Flow WEB output saves automatically into a user-authorized folder', async () => {
  ...
  assert.match(styles, /\.flow-queue-folder-progress\s*\{[\s\S]*max-width:\s*520px;[\s\S]*height:\s*4px;/)
  assert.match(styles, /\.flow-queue-kind-header\s*>\s*\.flow-queue-folder-actions\s*\{[\s\S]*min-width:\s*158px;/)
...
test('SRT image merge accepts a media folder without a timeline file', async () => {
  ...
  assert.match(page, /onLoad=\{\(\) => setMediaPreviewError\(false\)\}/)
  ...
  assert.match(page, /logo=\{\{ enabled: logoEnabled/)
