...
test('Automation queue UI is bilingual and keeps per-job controls', async () => {
  ...
  assert.match(page, /Mỗi dòng = 1 job riêng/)
  assert.match(page, /Each line = a separate job/)
  assert.match(page, /Link YouTube/)
  assert.match(page, /YouTube URL/)
  assert.match(page, /AI đề xuất 5 chủ đề/)
  assert.match(page, /AI suggest 5 topics/)
  assert.match(page, /Phân tích caption & viết lại/)
  assert.match(page, /Analyze captions & rewrite/)
  assert.match(styles, /\.automation-field-header\{/)
  assert.match(styles, /\.automation-suggest-topics-btn\{/)
  ...
  assert.match(page, /System prompt \(Bộ prompt 2D\)/)
  assert.match(page, /System prompt \(2D prompt engine\)/)
  assert.match(page, /v1\.0-base-vietnam-2D-image\.txt/)
  assert.match(page, /Tải file \.txt/)
  assert.match(page, /Upload \.txt/)
  assert.match(styles, /\.automation-prompt-box\{/)
  assert.match(styles, /\.automation-prompt-header\{/)
  assert.match(page, /settingsTab === 'flow' \? <div id="automation-settings-flow"[\s\S]*?<label className="automation-field-full"><span>\{t\('Tài khoản Flow'/)
  assert.doesNotMatch(page, /settingsTab === 'flow'[\s\S]*?Bộ prompt Audio-First 2D/)
  ...
  assert.match(page, /Hàng đợi Flow/)
  assert.match(page, /Flow queue/)
  assert.match(page, /automation-flow-queue-btn/)
  assert.match(styles, /\.automation-flow-queue-btn\{/)
  assert.match(page, /Ghép lại \/ Chọn đoạn chạy lại/)
  assert.match(page, /Recompose \/ Select restart stage/)
  assert.match(page, /Chỉ ghép lại video \(MP4\)/)
  assert.match(page, /Only recompose video \(MP4\)/)
  assert.match(page, /Toàn bộ video/)
  assert.match(page, /Entire video/)
  assert.match(page, /Chỉ ghép đoạn xem trước \(Preview\)/)
  assert.match(page, /Preview segment only/)
  assert.match(page, /Bắt đầu ghép lại/)
  assert.match(page, /Start recomposing/)
  assert.match(page, /automation-recompose-modal/)
  assert.match(styles, /\.automation-recompose-modal\{/)
  assert.match(styles, /\.automation-recompose-dialog\{/)
...
test('Flow requests interactive re-login when an account session is no longer valid', async () => {
  ...
  assert.match(page, /The Flow session has expired\. The profile was kept; click Reconnect in Accounts\./)
  assert.doesNotMatch(page, /Chrome is opening so you can sign in again/)
  assert.match(service, /def _session_needs_login/)
  assert.doesNotMatch(service, /self\.connect\(account\["id"\]\)/)
})
