...
export default function DrawingPage({ onBack }: { onBack: () => void }) {
  ...
  return <main className="drawing-page drawing-studio">
    <aside className="drawing-studio-rail" aria-label={t('Điều hướng Vẽ tay', 'Drawing navigation')}><nav><button id="drawing-nav-project" type="button" className={workspacePanel === 'project' ? 'is-active' : ''} onClick={() => setWorkspacePanel('project')} title={t('Dự án', 'Project')}><IconBatch size={19} /><span>{t('Dự án', 'Project')}</span></button><button id="drawing-nav-drawing" type="button" className={workspacePanel === 'drawing' ? 'is-active' : ''} onClick={() => setWorkspacePanel('drawing')} title={t('Vẽ tay', 'Drawing')}><IconVideo size={19} /><span>{t('Vẽ tay', 'Drawing')}</span></button></nav></aside>
