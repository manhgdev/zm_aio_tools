      ...
      {settingsOpen ? (
        <div id="batch-settings-panel" className="studio-settings-panel">
          <section className="studio-card studio-settings-common">
            ...
            <OutputFolderField isDesktopApp={isDesktopApp} value={outputDir} onChange={setOutputDir} onChoose={isDesktopApp ? async () => { const result = await studioApi.pickFolder(); if (result.path) setOutputDir(result.path); return result.path } : undefined} onSave={() => localStorage.setItem(BATCH_OUTPUT_DIR_LS, outputDir)} defaultPath={t('Ví dụ: du-an-01', 'Example: project-01')} appFolder={tab === 'review' ? 'review' : 'clone'} label={t('Thư mục xuất', 'Output folder')} />
          </section>
          {tab === 'review' ? (
